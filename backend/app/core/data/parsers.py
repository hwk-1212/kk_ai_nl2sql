...
支持:
  - Excel (.xlsx/.xls): 每个 Sheet → 一张 ParsedTable
  - CSV (.csv): 整个文件 → 一张 ParsedTable
  - SQLite (.sqlite/.db): 每个 table → 一张 ParsedTable
...
def coerce_value(val, col_type: str):
    """将原始值转换为 Python 对象，匹配 PG 列类型 (asyncpg 严格类型)。"""
    if val is None:
        return None
    if isinstance(val, (date, datetime)) and col_type in ("date", "timestamp"):
...
class FileParser:
    ...
    def _parse_csv(self, file_path: str, original_name: str | None = None) -> ParseResult:
            ...
            for row_values in reader:
                ...
                for j, val in enumerate(row_values):
                    if j < len(columns):
                        ...
                        row_dict[columns[j]] = val if val else None
                all_rows.append(row_dict)
        ...
        return ParseResult(tables=[ParsedTable(
            name=table_name, columns=column_infos, data=all_rows, row_count=len(all_rows),
        )])
