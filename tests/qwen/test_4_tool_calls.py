"""
测试4: 千问 qwen-plus Tool Calls
- 模型: qwen-plus
- 模式: tools 定义 + 多轮工具调用
- 目的: 记录 tool_calls 返回字段结构

测试包含:
  4a: 普通模式 tool call (非流式)
  4b: 流式 tool call
  4c: 思考模式 + tool call
"""
import json
...
def test_tool_call_normal():
    """4a: 普通模式 - Tool Call (非流式)"""
    print("=" * 80)
    print("测试4a: 千问 Tool Call - 普通模式（非流式）")
    print("=" * 80)
    ...
    if msg.tool_calls:
        ...
        print("\n[WARNING] 模型未返回 tool_calls，直接回答了")


def test_tool_call_stream():
    """4b: 流式 Tool Call"""
    print("\n" + "=" * 80)
    print("测试4b: 千问 Tool Call - 流式")
    print("=" * 80)

    messages = [{"role": "user", "content": "深圳现在天气如何？"}]
    ...
    if usage_chunk:
        print(f"\n--- usage chunk ---")
        print(json.dumps(usage_chunk, indent=2, ensure_ascii=False))


def test_tool_call_thinking():
    """4c: 思考模式 + Tool Call"""
    print("\n" + "=" * 80)
    print("测试4c: 千问 Tool Call - 思考模式")
    print("=" * 80)

    messages = [{"role": "user", "content": "帮我查一下北京和上海的天气，对比一下"}]
